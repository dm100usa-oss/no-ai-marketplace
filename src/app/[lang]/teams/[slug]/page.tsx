...
export function generateStaticParams() {
  const teams = baseProfiles.filter((p) => p.profileType === "team");
  return LOCALES.flatMap((lang) => teams.map((p) => ({ lang, slug: p.slug })));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${site.url}${localizedPath(locale, `/teams/${p.slug}`)}`,
      type: "profile",
    },
  };
}
...
export default async function TeamPage({
  ...
  const p = getProfileL(slug, locale);
  if (!p || p.profileType !== "team") notFound();
